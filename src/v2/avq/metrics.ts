...
/** pool: frames the chance baseline may draw from (default all), e.g. only off-grid frames for off-grid onsets. */
export function syncStats(eventsIdx: number[], peaks: Uint8Array, fps: number, seed = 7, pool?: number[]): SyncStats {
  ...
  for (let k = 0; k < 20; k++) {
    for (let m = 0; m < Math.max(ev.length, 10); m++) {
      const e = pool && pool.length ? pool[Math.floor(r() * pool.length)] : Math.floor(-lo + r() * Math.max(1, n - hi + lo - 1));
      if (e + lo < 0 || e + hi >= n) continue;
      if (hitAt(e) !== null) ch++;
      draws++;
    }
  }
...
export function melodyStats(c: ClipLike): MelodyStats {
  ...
  // No pitched melody in the clip (or none measured, e.g. live renders): unknown, not zero.
  const score = frames > fps * 2 ? clamp01((Math.abs(best.r) - nullR) / 0.35) : NaN;
  return { frames, r, best, nullR, score };
}
...
export function correspondences(c: ClipLike): CorrespondenceStats {
  ...
  const pitched = Array.from(pitch).filter(Number.isFinite).length > fps * 2;
  const rules: Record<string, number> = {
    'pitch>height': pitched ? pearson(s2(pitch), s2(c.col('cy'))) : NaN,
  ...
  const score = mean(Object.values(rules).filter(Number.isFinite).map((v) => clamp01(v / 0.5)));
  return { rules, score };
}
...
export interface ClockStats {
  /** Mean R^2 of the change features explained by bar phase alone (their average shape per bar). */
  clockShare: number;
  /** The same for the music (onset strength and stem envelopes): how bar-periodic the song itself is here. */
  musicClock: number;
  /** clockShare - musicClock, floored at 0: bar-locked motion the music does not account for. */
  excess: number;
  /**
   * Change depth: std / mean of the (smoothed) frame and thumbnail change. Steady motion (a
   * constant spin or sweep, whatever speed) has little depth; motion that swells and hits with the
   * music has a lot.
   */
  depth: number;
  /** Per feature R^2. */
...
/**
 * How much of the visual change is a pure beat / bar clock: each change feature is folded by
 * bar phase (32 bins over the bar, averaged across bars) and the folded shape is used as the
 * prediction. 1 = the motion is the same every bar whatever the music does (tempo-locked
 * spinning or sweeping); low = the motion varies from bar to bar (with the music, or noise).
 */
export function clockStats(c: ClipLike, tch: Float32Array): ClockStats {
  const bins = 32;
  const phase = c.col('barPhase');
  const feats: Record<string, Float32Array> = { diff: c.col('diff'), flowMag: c.col('flowMag'), curl: c.col('curl'), div: c.col('div'), lum: c.col('lum'), coverage: c.col('coverage'), thumbChange: tch };
  const byFeature: Record<string, number> = {};
  const fold = (x: Float32Array) => {
    const sum = new Float64Array(bins);
    const cnt = new Float64Array(bins);
    const b = (i: number) => Math.min(bins - 1, Math.max(0, Math.floor(phase[i] * bins)));
    for (let i = 0; i < c.n; i++) (sum[b(i)] += x[i]), cnt[b(i)]++;
    const m = mean(x);
    let ssr = 0, sst = 0;
    for (let i = 0; i < c.n; i++) {
      const pred = cnt[b(i)] ? sum[b(i)] / cnt[b(i)] : m;
      ssr += (x[i] - pred) ** 2;
      sst += (x[i] - m) ** 2;
    }
    // Small-sample bias: with n/bins points per bin, noise alone explains ~bins/n of the variance.
    if (sst < 1e-9) return NaN; // a constant feature says nothing about the clock
    return Math.max(0, (1 - ssr / sst - bins / c.n) / (1 - bins / c.n));
  };
  for (const [k, x] of Object.entries(feats)) byFeature[k] = fold(x);
  const on = Float32Array.from(c.col('onDrums'), (v, i) => Math.max(v, c.col('onBass')[i], c.col('onVocals')[i], c.col('onOther')[i]));
  const musicClock = mean([fold(on), ...(['drums', 'bass', 'vocals', 'other'] as const).map((k) => fold(c.col(k)))]);
  const clockShare = mean(Object.values(byFeature));
  const cv = (x: Float32Array) => {
    const sm = movingAverage(x, 3);
    const m = mean(sm);
    const sd = Math.sqrt(mean(Float32Array.from(sm, (v) => (v - m) ** 2)));
    return sd / (Math.abs(m) + 1e-4);
  };
  const depth = mean([cv(feats.diff), cv(tch)]);
  return { clockShare, musicClock: Number.isFinite(musicClock) ? musicClock : 0, excess: Math.max(0, clockShare - musicClock), depth, byFeature };
}
...
export interface ReportCard {
  ...
  headline: {
    ...
    interest: number;
    correspond: number;
    /** Audio-event drive: how much the picture depends on the music beyond its beat / bar clock (metronome counterfactual, else 1 - clock share). */
    events: number;
    overall: number;
  };
  /** offGrid: onsets more than 80 ms from any beat (syncopation, vocals, riffs); a clock-locked preset cannot land them. */
  sync: { all: SyncStats; hits: SyncStats; perStem: Record<string, SyncStats>; beats: SyncStats; offGrid: SyncStats };
  clock: ClockStats;
...
export const HEADLINE_WEIGHTS = { sync: 0.2, coupling: 0.12, hookRhyme: 0.14, melody: 0.07, structure: 0.11, flow: 0.1, interest: 0.08, correspond: 0.05, events: 0.13 };
...
export function reportCard(c: ClipLike, meta: { preset: string; song: string; clip: string; clipT0: number }): ReportCard {
  ...
  const bi = c.col('beatIndex');
  const bp = c.col('beatPhase');
  const beatLen = 60 / (c.bpm || 120);
  const offGrid = (i: number) => Math.min(bp[i], 1 - bp[i]) * beatLen > 0.08 && Number.isFinite(bi[i]);
  const offGridEv = all.filter(offGrid);
  const offPool: number[] = [];
  for (let i = 0; i < c.n; i++) if (offGrid(i)) offPool.push(i);
  const sync = { all: syncStats(all, R.peaks, fps), hits: syncStats(hitsEv, R.peaks, fps), perStem, beats: syncStats(beatEv, R.peaks, fps), offGrid: syncStats(offGridEv, R.peaks, fps, 7, offPool) };
  const clock = clockStats(c, tch);
  ...
  // Off-grid onsets count a third when there are enough of them: landing only on-grid hits is what a metronome does.
  const og = sync.offGrid.events >= 8 ? Math.max(0, sync.offGrid.lift) / 0.4 : NaN;
  const syncScore = Number.isFinite(og)
    ? clamp01((0.35 * Math.max(0, sync.all.lift)) / 0.5 + (0.35 * Math.max(0, sync.hits.lift)) / 0.6 + 0.3 * og)
    : clamp01((0.5 * Math.max(0, sync.all.lift)) / 0.5 + (0.5 * Math.max(0, sync.hits.lift)) / 0.6);
  const headline = {
    sync: syncScore,
    ...
    events: Math.min(clamp01((clock.depth - 0.1) / 0.4), clamp01(1 - clock.excess / 0.4)),
    overall: 0,
  };
  ...
  if (clock.depth < 0.22 && flow.stillness < 0.5) notes.push(`steady motion: the change swells only ${Math.round(clock.depth * 100)}% around its mean (constant spin or sweep; little of it follows audio events)`);
  if (clock.excess > 0.25) notes.push(`clock-locked: ${Math.round(clock.clockShare * 100)}% of the motion repeats every bar, against ${Math.round(clock.musicClock * 100)}% for the music`);
  return { preset: meta.preset, song: meta.song, clip: meta.clip, frames: c.n, headline, clock, sync, coupling, rhyme, melody, correspond, structure, flow, interest, notes };
}
...
export interface CfLike {
  motion: number;
  variants: { id: string; kind: string; rel: number; motionRel?: number; stem?: string; reaction?: number; beats?: number }[];
...
export interface CfSummary {
  ...
  content: number;
  /** Divergence against a metronome render (same beat grid, flat audio features), minus the chaos floor: audio-event drive of the look. */
  events: number;
  /**
   * The same for MOTION only: how differently the picture moves frame to frame under the
   * metronome (relative to its own frame-to-frame change), minus the chaos floor. ~0 = the
   * motion is the clock's (a constant spin, a bar-locked sweep), even if colours follow the music.
   */
  motionEvents: number;
...
export function cfSummary(cf: CfLike): CfSummary {
  ...
  return {
    desync, chaos, syncSensitivity, content: find((v) => v.kind === 'offset'), events: Math.max(0, find((v) => v.kind === 'metronome') - floor),
    motionEvents: Math.max(0, (cf.variants.find((v) => v.kind === 'metronome')?.motionRel ?? NaN) - (cf.variants.find((v) => v.kind === 'gain')?.motionRel ?? 0)),
    stems, reactions,
