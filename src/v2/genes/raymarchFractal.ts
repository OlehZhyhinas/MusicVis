...
const boxRadius = new Map<string, number>();

/**
 * Natural bounding radius of a fractal at these settings (before the world scale). The Mandelbox's
 * size depends strongly on its scale (negative scales are much smaller than the cube bound
 * 2 (s + 1) / (s - 1)), so it is measured once per setting by marching in from that bound along 26
 * directions with the estimator.
 */
function naturalRadius(kind: FractalKind, scale: number, fold: number, iter: number): number {
  if (kind === 'mandelbulb') return 1.25;
  if (kind === 'menger') return Math.sqrt(3);
  const key = `${Math.round(scale * 20)}:${Math.round(fold * 20)}:${iter}`;
  const hit = boxRadius.get(key);
  if (hit !== undefined) return hit;
  const s = Math.abs(scale);
  const bound = ((2 * (s + 1)) / (s - 1)) * Math.sqrt(3);
  const f: FractalFrame = { kind, S: 1, radius: 1, scale, fold, power: 8, iter, rot: [1, 0, 0, 0, 1, 0, 0, 0, 1] };
  let best = 0.5;
  for (let dx = -1; dx <= 1; dx++) for (let dy = -1; dy <= 1; dy++) for (let dz = -1; dz <= 1; dz++) {
    if (!dx && !dy && !dz) continue;
    const l = Math.hypot(dx, dy, dz);
    let r = bound;
    for (let k = 0; k < 200 && r > best; k++) {
      const d = fractalDE(f, (dx / l) * r, (dy / l) * r, (dz / l) * r);
      if (d < 0.01 * bound) break;
      r -= Math.max(d, 0.005 * bound);
    }
    best = Math.max(best, r);
  }
  const out = Math.min(bound, best * 1.05);
  boxRadius.set(key, out);
  return out;
}
...
export function fractalFrame(kind: FractalKind, P: (k: string) => number, raw: Params, size: number, bass: number, seed: number, T: number): FractalFrame {
  ...
  // Measured at the unmodulated fold so the world scale does not jump with the bass.
  const S = radius / naturalRadius(kind, scale, P('fold'), raw.iter);
