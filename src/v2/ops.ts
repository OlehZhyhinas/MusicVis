...
const KIND_WEIGHTS: Partial<Record<Locus, Record<string, number>>> = {
  shape: { dot: 3, polygon: 1.5, star: 1.2, segment: 0.8, solid: 1.5, bars: 1, curve: 2, plasma: 0.8, aurora: 0.8, terrain: 0.5, edge: 0.6, flame: 1, superscope: 1.5, beams: 0.7, scene: 0.6, cells: 0.7, cymatics: 0.7, landscape: 0.5, tonnetz: 0.5 },
...
export function randomGene(locus: Locus, rng: Rng, kind?: string): Gene {
  ...
  if (locus === 'emit' && k === 'ecosystem') p.count = Math.min(p.count, 65536);
...
const MUTATORS: [number, string, Mutator][] = [
  ...
  // Harmony (chord progression -> symmetry): rarely gained (or lost), nudged when present.
  [0.5, 'harmony', (g, rng) => {
    if (!g.harmony) g.harmony = randomHarmony(rng);
    else if (rng() < 0.3) delete g.harmony;
    else jitterHarmony(g.harmony, rng);
    return true;
  }],
