 ...
 * response curve and clock, a reaction's curve); 6: genomes of format 5 (colour in parts: palette,
 * per-body colour mapping, tone). Older genomes are converted on load by
...
export class Population {
  ...
  static fromJSON(data: unknown): Population {
    ...
    for (const raw of d.members) {
      ...
      if ((!old && rg.v !== 3 && rg.v !== 4 && rg.v !== 5) || !Array.isArray(parts) || !parts.length) continue;
      let g: Genome;
      try {
        g = repair(rg);
      } catch {
