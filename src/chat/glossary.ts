...
export const ENTRIES: Record<string, string> = {
  ...
  'shape.scene': 'scene(ray-marched 3D,1/preset): scene 0 melting shapes,1 endless lattice of shapes between two plates(gap=corridor height,spec=how far cells rise with their spectrum band),2 ribbed tunnel flown through(gap=how much it bends,spec=ribs close in with the spectrum); cam 0 orbit,1 fly,2 dolly zoom; roam=camera travel; res=render res(cost); size=object scale; blend=melt; speed=animation+camera speed; pulse=bass swell; kick=drum camera jolt; vary=section reshuffle; rim/ao/fog/glow=light. Material sets the look(line=neon rims, glow=haze, chrome=reflective).',
