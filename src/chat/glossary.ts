...
export const ENTRIES: Record<string, string> = {
  ...
  'shape.tonnetz': 'tonnetz(the harmony drawn as a tonal lattice, 1 copy): fifths run right, major thirds up-right, each small triangle is a chord(up=major,down=minor); note nodes glow with the notes sounding; the current chord triangle lights, the last chords leave a glowing path. scale=lattice step; follow=camera follows the walk(0=fixed on home); tilt=rotation(turns); nodes/lines/fill/echo(same chord elsewhere)/trail=brightness of each part; pulse=flash on chord change and resolution.',
  'shape.notes': 'notes(the melody drawn as it is played, 1 copy): time runs along a lane or round a ring, pitch across; held/gliding notes are a ribbon bending with the pitch (vibrato shimmers), each note start stamps a mark that fades when the note ends; legato music favours the ribbon, staccato the marks. mode 0 lane,1 ring; span=seconds shown; len=lane length/ring size; height=pitch range; now=where the present sits; tilt=rotation(turns); ribbon/thick=held-note brightness/width; marks=note-start marks; form 0 dot,1 spark,2 bar as long as the note,3 ripple; size; fade=seconds a mark fades after its note ends; rise=marks drift as they age; shimmer=vibrato sparkle; hues=hue spread over pitch; glow=halo.',
  ...
  'gene.timbre': 'TIMBRE(optional): the sound\'s timbre becomes the bodies\' material. src=whose timbre(0 mix,1 drums,2 bass,3 vocals,4 other); bright sound=metallic sheen+highlight(sheen); pure tones=clear glass with a glowing rim(glass); noisy/distorted=sparkling grit(grain; scale=grain size); breathy=soft velvet bloom(velvet); sharp attacks flash outlines(edge); emboss=whole picture lit as a surface by roughness(glossy/metallic when bright).',
