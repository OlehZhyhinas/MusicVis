...
CHOREO(optional, whole song, uses the known future of the song): before each drop the camera pushes in and leans, colour drains and light dims, then it slams on the drop and settles. lead=bars of build-up; curve=how late it bites; push=zoom-in; roll=lean(turns); drain=desaturate; dim=darken; punch=drop slam; relax=bars to settle. Scenes: each section type (verse, chorus, drop...) gets its own framing; frame=how far framings push/pan/lean; shot=which set of framings; glide=bars to move into a new framing(0=hard cut); dolly=slow push-in across each section; scene=hue shift per section type. arc=push-in swelling across each phrase of phrase bars.
