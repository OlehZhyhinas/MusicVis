...
export function raymarchChecks(check: Check): void {
  ...
  // Fractals: every kind builds its estimator into the scene pass, and a maxed-out fractal is repaired under budget.
  const fracBad: string[] = [];
  for (let fk = 0; fk < FRACTAL_KINDS.length; fk++) {
    const g = cloneGenome(r01);
    Object.assign(g.bodies[0].shape.p, { scene: FRACTAL_FIRST + fk, iter: 10, res: 0.7 });
    g.chain = Array.from({ length: 4 }, () => ({ op: 'noise' as const, stage: 'warp' as const, w: 1, p: { amp: 0.001, scale: 2, speed: 0.3 } }));
    const r = repair(g);
    const src = buildSources(r).scene ?? '';
    if (validate(r).length) fracBad.push(`${FRACTAL_KINDS[fk]}: ${validate(r)[0]}`);
    if (!(estimateCost(r) < COST_BUDGET_MS)) fracBad.push(`${FRACTAL_KINDS[fk]}: cost ${estimateCost(r).toFixed(2)}`);
    if (!src.includes('rmMandelbox') || !src.includes('rmBulb') || !src.includes('rmMenger')) fracBad.push(`${FRACTAL_KINDS[fk]}: glsl`);
  }
  check('scene.fractals', !fracBad.length, fracBad.join(' | ') || `${FRACTAL_KINDS.join(', ')} build, and heavy settings are repaired under budget (res, then iterations)`);
